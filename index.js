...
app.post('/prod', async (req, res) => {
  try {
    ...
    const [result] = await pool.query('INSERT INTO usuario (nome, preco, descricao) VALUES (?, ?, ?)', [nome, preco, descricao]);
    res.json({ id: result.insertId, nome:nome, preco:preco, descricao:descricao});
  } catch (error) {
    console.error("Erro ao criar usuário:", error);
    res.status(500).send("Erro ao criar usuário");
  }
...
app.get('/users/:id', async (req, res) => {
  try {
    ...
    res.status(500).send("Erro ao buscar usuário3");
  }
});
